...
def _hparams(algorithm, dataset, random_seed):
    ...
    if algorithm == "DAMP":
        _hparam("lambda_u", 0.1, lambda r: r.uniform(0.0, 1.0))
        _hparam("tau", 0.7, lambda r: r.choice([0.5, 0.7, 0.9]))
        _hparam("lr", 1e-6, lambda r: 1e-6)
        _hparam("weight_decay", 1e-4, lambda r: 10 ** r.uniform(-6, -2))
        _hparam("clip_model_name", "ViT-B/16", lambda r: "ViT-B/16")
        _hparam("text_prompt_length", 5, lambda r: int(r.choice([4, 5, 6])))
        _hparam("visual_prompt_length", 5, lambda r: int(r.choice([4, 5, 6])))
        _hparam("cross_attn_dim", 512, lambda r: int(r.choice([256, 512, 768])))
        _hparam("cross_attn_heads", 8, lambda r: int(r.choice([4, 8])))
        _hparam("lambda_sc", 0.5, lambda r: r.uniform(0.5, 2.0))
        _hparam("lambda_inst", 0.1, lambda r: r.uniform(0.0, 1.0))
        _hparam("ntxent_temp", 0.1, lambda r: r.choice([0.05, 0.1, 0.2]))
        _hparam("freeze_visual", True, lambda r: bool(r.choice([True, False])))
        _hparam("freeze_text", True, lambda r: bool(r.choice([True, False])))
