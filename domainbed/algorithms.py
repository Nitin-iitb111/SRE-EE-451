...
# class DAMP(Algorithm):
#     """
#     Domain-Agnostic Mutual Prompting (DAMP)
#     Enhanced with:
#       - CLIPWrapper integration
#       - Cross-attention fusion
#       - Self-consistency loss
#       - NT-Xent instance contrastive loss
#     """

#     def __init__(self, input_shape, num_classes, num_domains, hparams):
#         super(DAMP, self).__init__(input_shape, num_classes, num_domains, hparams)
#         self.num_classes = num_classes
#         self.hparams = hparams

#         # Load CLIP
#         self.clip = networks.CLIPWrapper(
#             model_name=hparams.get("clip_model_name", "ViT-B/32"),
#             device=hparams.get("device", "cuda" if torch.cuda.is_available() else "cpu"),
#             freeze_visual=hparams.get("freeze_visual", True),
#             freeze_text=hparams.get("freeze_text", False)
#         )
#         self.device = self.clip.device
#         self.clip.classnames = [str(i) for i in range(num_classes)]

#         # Prompt parameters
#         txt_len = hparams.get("text_prompt_length", 5)
#         vis_len = hparams.get("visual_prompt_length", 5)
#         text_dim = self.clip.text_embedding_dim
#         visual_dim = self.clip.visual_embedding_dim

#         self.text_prompt = nn.Parameter(torch.randn(txt_len, text_dim) * 0.02)
#         self.visual_prompt = nn.Parameter(torch.randn(vis_len, visual_dim) * 0.02)

#         # Projection layers (optional)
#         cross_dim = hparams.get("cross_attn_dim", visual_dim)
#         if cross_dim != visual_dim:
#             self.image_proj = nn.Linear(visual_dim, cross_dim)
...
#             embed_dim=cross_dim,
#             num_heads=hparams.get("cross_attn_heads", 8),
#             batch_first=True
#         )

#         # Classifier (optional)
#         self.classifier = nn.Linear(cross_dim, num_classes)

#         # Logit scaling
#         self.logit_scale = nn.Parameter(torch.tensor(self.clip.logit_scale).log())

#         # Optimizer
#         self.optimizer = torch.optim.Adam(
#             filter(lambda p: p.requires_grad, self.parameters()),
#             lr=hparams.get("lr", 1e-6),
#             weight_decay=hparams.get("weight_decay", 1e-4)
#         )
#         try:
#             from domainbed.networks import get_class_names_from_dataset
#             if isinstance(hparams, dict) and 'dataset_obj' in hparams:
#                 class_names = get_class_names_from_dataset(hparams['dataset_obj'])
#                 self.clip.classnames = class_names
#             else:
...
#         self._cached_class_embeds = None
#         self._update_class_embeds()

#     # --- Core methods ---

#     def _update_class_embeds(self):
#         """Compute and cache text embeddings for each class once."""
#         with torch.no_grad():
#             self._cached_class_embeds = self.clip.encode_text_with_prompt(
#                 self.clip.classnames, self.text_prompt
#             )

#     def _maybe_project(self, x):
#         """Project embeddings if projection layers exist."""
#         if self.image_proj is not None:
#             return self.image_proj(x)
#         return x

#     # --- NT-Xent instance loss ---

#     def nt_xent_loss(self,feats, labels, temperature=0.1, eps=1e-8):
#         """
#         Compute NT-Xent (Normalized Temperature-scaled Cross Entropy) loss.
#         - feats: (N, D) normalized feature embeddings.
#         - labels: (N,) integer tensor of class IDs.
#         """
#         if not isinstance(feats, torch.Tensor):
#             raise ValueError("Expected feats to be a tensor, got {}".format(type(feats)))
#         device = feats.device
#         N = feats.shape[0]
...
#         sim = sim - torch.eye(N, device=device) * 1e9  # mask self-similarity

#         # Build mask for positives (same class, not self)
#         labels = labels.contiguous().view(-1, 1)
#         mask = torch.eq(labels, labels.T).float().to(device)
#         mask.fill_diagonal_(0)

#         # If no positive pairs, return zero loss (avoid NaN)
#         pos_count = mask.sum()
#         if pos_count < 1:
...
#         # For positives, take mean log prob
#         loss = -(mask * log_prob).sum(dim=1) / (mask.sum(dim=1) + eps)
...
#         device = self.device
#         all_x, all_y = [], []
#         for x, y in tqdm(minibatches, desc="[DAMP] Loading domains", leave=False):
#             all_x.append(x)
#             all_y.append(y)
...
#         # 1️⃣ Encode images and text
#         image_feats = self.clip.encode_image_with_prompt(all_x, self.visual_prompt)
#         text_class_embeds = self._cached_class_embeds

#         # 2️⃣ Projection
#         if self.image_proj is not None:
#             image_feats_proj = self.image_proj(image_feats)
#             text_embeds_proj = self.text_proj(text_class_embeds)
#         else:
#             image_feats_proj = image_feats
#             text_embeds_proj = text_class_embeds

#         # 3️⃣ Cross-attention fusion
#         query = image_feats_proj.unsqueeze(1)
#         key_value = text_embeds_proj.unsqueeze(0).expand(B, -1, -1)
#         attn_output, _ = self.cross_attn(query=query, key=key_value, value=key_value)
#         attn_output = attn_output.squeeze(1)

#         # 4️⃣ Compute logits
#         img_norm = F.normalize(attn_output, dim=1)
#         txt_norm = F.normalize(text_embeds_proj, dim=1)
#         logits = torch.matmul(img_norm, txt_norm.T) * self.logit_scale.exp()

#         # 5️⃣ Compute losses
#         loss_sup = F.cross_entropy(logits, all_y)
#         loss_sc = F.mse_loss(
#             self._maybe_project(image_feats),
#             self._maybe_project(attn_output.detach())
#         )
#         lambda_inst = self.hparams.get("lambda_inst", 0.0)
#         loss_inst = torch.tensor(0.0, device=device)
#         if lambda_inst > 0:
#             with torch.no_grad():
#                 img_proj = F.normalize(self._maybe_project(image_feats), dim=1)
#                 attn_proj = F.normalize(self._maybe_project(attn_output), dim=1)
#                 temperature=self.hparams.get("ntxent_temp", 0.1)
#                 loss_inst = self.nt_xent_loss(
#             torch.cat([img_proj, attn_proj], dim=0),
#             torch.cat([all_y, all_y], dim=0),
#             temperature  # Ensure this is the only place temperature is set
#         )

#         λ_sc = self.hparams.get("lambda_sc", 1.0)
#         λ_inst = self.hparams.get("lambda_inst", 0.5)

#         loss = loss_sup + λ_sc * loss_sc + λ_inst * loss_inst
...
#         x = x.to(device)
#         with torch.no_grad():
#             image_feats = self.clip.encode_image_with_prompt(x, self.visual_prompt)
#             text_class_embeds = self._cached_class_embeds
...
#             img_norm = F.normalize(image_feats_proj, dim=1)
#             txt_norm = F.normalize(text_embeds_proj, dim=1)
#             logits = torch.matmul(img_norm, txt_norm.T) * self.logit_scale.exp()
#             return logits
...
class DAMP(Algorithm):
    """
    Domain-Agnostic Mutual Prompting (DAMP) implemented for DomainBed.

    Key points:
      - Safe AMP: only used when CUDA available. GradScaler created conditionally.
      - Ensures all optimizer params live on the same device before creating optimizer.
      - Robust NT-Xent instance loss (same-class positives).
      - Pseudo-label handling (if unlabeled / uda data passed).
      - Periodic cached text embedding refresh to reflect prompt changes.
    """
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.hparams = hparams
        # Decide device early
        self.device = torch.device(hparams.get("device", "cuda" if torch.cuda.is_available() else "cpu"))
        self.num_classes = num_classes
        self.update_count = 0
        ...
        self.clip = networks.CLIPWrapper(
            ...
            freeze_visual=hparams.get("freeze_visual", True),
            freeze_text=hparams.get("freeze_text", False),
            classnames=None,  # will fill below
            verbose=False
        )

        # ✅ Auto-extract proper class names from dataset if provided
        classnames = None
        dataset_obj = hparams.get("dataset_obj", None)
        if dataset_obj is not None:
            try:
                # Most DomainBed datasets expose .datasets[0].classes
                if hasattr(dataset_obj, "datasets") and hasattr(dataset_obj.datasets[0], "classes"):
                    classnames = dataset_obj.datasets[0].classes
                elif hasattr(dataset_obj, "classes"):
                    classnames = dataset_obj.classes
            except Exception:
                pass
        ...
        # ✅ Update CLIP class names
        self.clip.classnames = classnames


        # --- Prompts ---
        txt_len = int(hparams.get("text_prompt_length", 5))
        vis_len = int(hparams.get("visual_prompt_length", 5))
        text_dim = int(self.clip.text_embedding_dim)
        visual_dim = int(self.clip.visual_embedding_dim)

        # learnable prompt embeddings
        self.text_prompt = nn.Parameter(torch.randn(txt_len, text_dim) * 0.02)
        self.visual_prompt = nn.Parameter(torch.randn(vis_len, visual_dim) * 0.02)

        # --- Projection and cross-attention ---
        cross_dim = int(hparams.get("cross_attn_dim", visual_dim))
        heads = int(hparams.get("cross_attn_heads", 8))
        self.image_proj = nn.Linear(visual_dim, cross_dim) if cross_dim != visual_dim else None
        self.text_proj = nn.Linear(text_dim, cross_dim) if cross_dim != text_dim else None

        # cross-attention used for fusion
        self.cross_attn = nn.MultiheadAttention(embed_dim=cross_dim, num_heads=heads, batch_first=True)

        # Optional classifier (unused by default, kept for flexibility)
        self.classifier = nn.Linear(cross_dim, num_classes)

        # Logit scaling param (initialized from CLIP's logit_scale)
        # store as Parameter to allow small tuning if desired
        try:
            init_logit = float(self.clip.logit_scale)
        except Exception:
            init_logit = 1.0
        self.logit_scale = nn.Parameter(torch.tensor(init_logit).log())

        # Move whole module to device (ensures prompt tensors are on device)
        self.to(self.device)
        try:
            # also ensure clip.model is on device
            self.clip.model.to(self.device)
        except Exception:
            pass

        # Decide AMP usage only if CUDA is available
        self.use_amp = (self.device.type == "cuda") and torch.cuda.is_available()
        self.scaler = GradScaler() if self.use_amp else None

        # Cache class embeddings (on device)
        self._cached_class_embeds = None
        self._update_class_embeds()
        print(f"[DAMP] Loaded {len(self.clip.classnames)} classnames for CLIP text encoder.")


        # --- Build optimizer AFTER moving everything to device & caching class embeds ---
        params = list(filter(lambda p: p is not None and p.requires_grad, self.parameters()))

        # Optionally include CLIP encoder params if user wants to fine-tune them
        if not bool(hparams.get("freeze_text", False)):
            try:
                params += [p for p in self.clip.model.transformer.parameters() if p.requires_grad]
            except Exception:
                pass
        if not bool(hparams.get("freeze_visual", True)):
            try:
                params += [p for p in self.clip.model.visual.parameters() if p.requires_grad]
            except Exception:
                pass

        # deduplicate params and ensure they are on same device
        unique_params = []
        seen = set()
        for p in params:
            pid = id(p)
        ...
        # Ensure params on same device: attempt to fix by moving module to device if needed
        param_devices = {p.device for p in unique_params}
        if len(param_devices) != 1:
            # Attempt to move everything to target device and re-evaluate
            self.to(self.device)
            try:
                self.clip.model.to(self.device)
            except Exception:
            ...
            unique_params = []
            seen = set()
            params_after = list(filter(lambda p: p is not None and p.requires_grad, self.parameters()))
            ...
            param_devices = {p.device for p in unique_params}
            if len(param_devices) != 1:
                raise RuntimeError(f"[DAMP] Optimizer parameters are on multiple devices: {param_devices}")

        # finally create optimizer
        lr = float(hparams.get("lr", 5e-5))
        weight_decay = float(hparams.get("weight_decay", 1e-4))
        self.optimizer = torch.optim.Adam(unique_params, lr=lr, weight_decay=weight_decay)

        # safe defaults for hyperparams we reference
        self.hparams.setdefault("lambda_sc", 1.0)
        self.hparams.setdefault("lambda_inst", 0.0)
        self.hparams.setdefault("lambda_u", 0.0)
        self.hparams.setdefault("tau", 0.7)
        self.hparams.setdefault("ntxent_temp", 0.1)

    # ---------------------------
    #  Helper: cached class embeds
    # ---------------------------
    def _update_class_embeds(self):
        """Compute text embeddings for classnames with current text_prompt and cache them."""
        with torch.no_grad():
            emb = self.clip.encode_text_with_prompt(self.clip.classnames, self.text_prompt)
        self._cached_class_embeds = emb.to(self.device).float()
        # normalize cached embeddings in advance for stable dot-product logits
        self._cached_class_embeds = F.normalize(self._cached_class_embeds, dim=1)

    # ---------------------------
    #  Robust NT-Xent (class positives)
    # ---------------------------
    def _nt_xent_instance_loss(self, embeddings, labels, temperature=0.1, eps=1e-8):
        """
        embeddings: (N, D) raw or normalized
        labels: (N,)
        Returns scalar loss (0 if no positive pairs).
        """
        ...
        # remove self-similarity by setting large negative value
        diag_mask = torch.eye(N, dtype=torch.bool, device=device)
        sim = sim.masked_fill(diag_mask, -1e12)
        ...
        loss_per = -torch.log((numerator[valid] + eps) / (denom[valid] + eps))
    ...
    # ---------------------------
    def predict(self, x):
        """Return logits for input x (tensor)."""
        x = x.to(self.device)
        with torch.no_grad():
            img_feats = self.clip.encode_image_with_prompt(x, self.visual_prompt)
            if self.image_proj is not None:
                img_proj = self.image_proj(img_feats)
            else:
                img_proj = img_feats
            img_norm = F.normalize(img_proj, dim=1)
            logits = img_norm @ self._cached_class_embeds.t()
            logits = logits * self.logit_scale.exp()
        return logits

    # ---------------------------
    #  DomainBed update signature
    #  Accepts: minibatches = list of (x, y) for each source domain
    #           uda_device (unused or can be used for unlabeled device)
    # ---------------------------
    def update(self, minibatches, uda_device=None):
        """
        minibatches: list of (x, y) tuples for each source domain (already on device by DomainBed's caller).
        uda_device: either None or list/structure for unlabeled data (DomainBed passes 'uda_device').
        """
        self.update_count += 1
        self.train()
        ...
        # concat supervised minibatches
        all_x = torch.cat([x for (x, y) in minibatches]).to(device)
        all_y = torch.cat([y for (x, y) in minibatches]).to(device)
        B = all_x.shape[0]

        # dynamic scaling of some lambdas (warmup)
        lambda_sc = float(self.hparams.get("lambda_sc", 1.0)) * min(1.0, self.update_count / 1000.0)
        lambda_inst = float(self.hparams.get("lambda_inst", 0.0))
        lambda_u = float(self.hparams.get("lambda_u", 0.0))
        tau = float(self.hparams.get("tau", 0.7))
        ntxent_temp = float(self.hparams.get("ntxent_temp", 0.1))

        # ---------- Forward (use autocast only when use_amp True) ----------
        if self.use_amp:
        ...
        with autocast_ctx():
            # encode image features (with visual prompt injection)
            image_feats = self.clip.encode_image_with_prompt(all_x, self.visual_prompt)  # (B, D_vis)

            # prepare text class embeddings (cached)
            text_class_embeds = self._cached_class_embeds  # (C, D_txt) normalized already
            ...
            # normalize projections for stable attention / logits
            image_feats_proj_norm = F.normalize(image_feats_proj, dim=1)
            text_embeds_proj_norm = F.normalize(text_embeds_proj, dim=1)

            # bidirectional cross-attention
            # image->text
            query = image_feats_proj_norm.unsqueeze(1)            # (B, 1, D)
            key_value = text_embeds_proj_norm.unsqueeze(0).expand(B, -1, -1)  # (B, C, D)
            attn_i2t, _ = self.cross_attn(query=query, key=key_value, value=key_value)  # (B,1,D)
            # text->image (attending from text tokens to image query), average across class tokens
            attn_t2i, _ = self.cross_attn(query=key_value, key=query, value=query)  # (B, C, D)
            attn_output = 0.5 * (attn_i2t.squeeze(1) + attn_t2i.mean(dim=1))  # (B, D)

            # logits (supervised)
            logits = (F.normalize(attn_output, dim=1) @ text_embeds_proj_norm.t()) * self.logit_scale.exp()
            loss_sup = F.cross_entropy(logits, all_y)

            # self-consistency (MSE) between projected image features and attended output
            loss_sc = F.mse_loss(F.normalize(image_feats_proj, dim=1), F.normalize(attn_output.detach(), dim=1))

            # instance contrastive (NT-Xent) done on concatenated [img_proj ; attn_output]
            # We run it only if lambda_inst > 0 to avoid extra compute
            if lambda_inst > 0:
                # concat img and attn as positive pairs (img_i <-> attn_i) but we want same-class positives across the whole set
                concat_feats = torch.cat([F.normalize(image_feats_proj, dim=1), F.normalize(attn_output, dim=1)], dim=0)
                concat_labels = torch.cat([all_y, all_y], dim=0)
                loss_inst = self._nt_xent_instance_loss(concat_feats, concat_labels, temperature=ntxent_temp)
            else:
                loss_inst = torch.tensor(0.0, device=device)

            # ---------- Unlabeled / pseudo-label handling ----------
            loss_u = torch.tensor(0.0, device=device)
            if lambda_u > 0 and uda_device is not None:
                # DomainBed often passes uda_device as list of unlabeled minibatches on device.
                # Try to accept both a list of (x_u, _) or a single minibatch
                try:
                    # If uda_device is list-like, collect tensors
                    if isinstance(uda_device, (list, tuple)):
                        # assume each element is (x_u, _) or just x_u
                        xu_list = []
                        for ub in uda_device:
                            if isinstance(ub, (list, tuple)) and len(ub) > 0:
                                xu_list.append(ub[0])
                            else:
                                xu_list.append(ub)
                        x_u = torch.cat(xu_list).to(device)
                    else:
                        # single minibatch (x_u, _)
                        if isinstance(uda_device, (list, tuple)) and len(uda_device) >= 1:
                            x_u = uda_device[0].to(device)
                        else:
                            x_u = uda_device.to(device)
                except Exception:
                    # fallback: skip unlabeled
                    x_u = None

                if x_u is not None and x_u.numel() > 0:
                    with torch.no_grad():
                        feats_u = self.clip.encode_image_with_prompt(x_u, self.visual_prompt)  # (Nu, D)
                        if self.image_proj is not None:
                            feats_u_proj = F.normalize(self.image_proj(feats_u), dim=1)
                        else:
                            feats_u_proj = F.normalize(feats_u, dim=1)
                        # logits using cached text embeds projection (text_embeds_proj_norm)
                        pseudo_logits = (feats_u_proj @ text_embeds_proj_norm.t()) * self.logit_scale.exp()
                        probs = F.softmax(pseudo_logits, dim=1)
                        confs, pseudo_labels = probs.max(dim=1)
                        mask = confs.ge(tau)
                        if mask.sum() > 0:
                            logits_u_model = self.predict(x_u)  # returns logits
                            loss_u = (F.cross_entropy(logits_u_model, pseudo_labels, reduction="none") * mask.float()).sum() / (mask.sum().float() + 1e-8)
                        else:
                            loss_u = torch.tensor(0.0, device=device)

        # ---------- Backward & step (two paths: AMP on CUDA, or vanilla FP32) ----------
        self.optimizer.zero_grad()
        if self.use_amp and self.scaler is not None:
            # AMP path
            self.scaler.scale(loss_sup + lambda_sc * loss_sc + lambda_inst * loss_inst + lambda_u * loss_u).backward()
            # gradient clipping on unscaled grads is done after unscaling internally by scaler.step
            torch.nn.utils.clip_grad_norm_(self.parameters(), 5.0)
            # step via scaler
            try:
                ...
                # If scaler.step fails (rare), fall back to unscaled flow (raise to debug)
                print("[DAMP] scaler.step() failed — falling back. Error:", e)
                # Unscale manually and step (only safe if params on CUDA)
                self.scaler.unscale_(self.optimizer)
                torch.nn.utils.clip_grad_norm_(self.parameters(), 5.0)
                self.optimizer.step()
        else:
            # plain FP32 path
            total_loss = (loss_sup + lambda_sc * loss_sc + lambda_inst * loss_inst + lambda_u * loss_u)
            total_loss.backward()
            torch.nn.utils.clip_grad_norm_(self.parameters(), 5.0)
            self.optimizer.step()

        # periodic refresh of cached class embeddings (so prompt changes affect text embeddings)
        if self.update_count == 1 or (self.update_count % int(self.hparams.get("refresh_text_every", 500))) == 0:
            self._update_class_embeds()


        # metrics (ensure tensors to float)
        return {
            "loss": float((loss_sup + lambda_sc * loss_sc + lambda_inst * loss_inst + lambda_u * loss_u).detach().cpu().item()),
            "loss_sup": float(loss_sup.detach().cpu().item()),
            "loss_sc": float(loss_sc.detach().cpu().item()),
            "loss_inst": float(loss_inst.detach().cpu().item()) if isinstance(loss_inst, torch.Tensor) else float(loss_inst),
            "loss_u": float(loss_u.detach().cpu().item()) if isinstance(loss_u, torch.Tensor) else float(loss_u),
        }
